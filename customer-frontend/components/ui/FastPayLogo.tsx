...
export const FastPayLogo: React.FC<FastPayLogoProps> = ({ className = '', size = 'md' }) => {
  const sizes = {
    sm: { icon: 'w-7 h-7 rounded-lg border-1.5 p-0.5', text: 'text-lg', dot: 'r-5' },
    md: { icon: 'w-9 h-9 rounded-xl border-2 p-1', text: 'text-xl', dot: 'r-6' },
    lg: { icon: 'w-12 h-12 rounded-2xl border-2 p-1.5', text: 'text-2xl', dot: 'r-7' },
  };
    ...
    <div className={`flex items-center space-x-2.5 ${className}`}>
      ...
      <div className={`${s.icon} bg-white border-[#2563eb] shadow-xs flex items-center justify-center shrink-0`}>
        <svg viewBox="0 0 100 100" fill="none" className="w-full h-full text-[#2563eb]">
          {/* Stylized S-curve Lightning path */}
          <path
