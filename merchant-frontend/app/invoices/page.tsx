  ...
  useEffect(() => {
    async function fetchInvoices() {
      try {
        ...
        const res = await fetch(`${API_BASE_URL}/api/v1/merchant/invoices`, {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/v1/merchant/invoices`, {
        ...
        body: JSON.stringify({ total: parseInt(amount, 10) * 100 })
      });
      const data = await res.json();
      if (data.data) {
              ...
              {loading ? (
                <tr>
                  <td colSpan={4} className="py-8 text-center text-gray-400 text-xs">
                    Loading live merchant invoices...
                  </td>
                </tr>
              ) : invoices.length === 0 ? (
                <tr>
                  <td colSpan={4} className="py-8 text-center text-gray-400 text-xs">
                    No invoices generated yet. Click "Create New Invoice" to issue one.
                  </td>
                </tr>
              ) : (
                invoices.map((inv) => (
                  <tr key={inv.id} className="hover:bg-gray-50/60 transition-colors">
                    <td className="py-3.5 px-4 font-mono font-bold text-blue-600">{inv.invoiceNumber || inv.id}</td>
                    <td className="py-3.5 px-4 font-extrabold text-gray-900">₹{((inv.amount || inv.total || 0) / 100).toFixed(2)}</td>
                    <td className="py-3.5 px-4">
                      <span className="px-2 py-0.5 rounded text-[10px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200">
                        {inv.status || 'ISSUED'}
                      </span>
                    </td>
                    <td className="py-3.5 px-4 text-gray-500 text-[11px]">{inv.dueDate}</td>
                  </tr>
                ))
              )}
            </tbody>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <form onSubmit={handleCreate} className="bg-white border border-gray-200 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl p-6 space-y-4">
            <div className="flex items-center justify-between border-b border-gray-100 pb-3">
              <h3 className="font-bold text-sm text-gray-900">Issue New Invoice</h3>
            ...
            <div className="space-y-3 text-xs">
              <div>
                <label className="block font-bold text-gray-700 mb-1">Invoice Total Amount (₹) *</label>
                <input
                  type="number"
                  value={amount}
